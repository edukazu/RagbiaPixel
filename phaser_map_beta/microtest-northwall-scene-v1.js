(() => {
  ...
  class MicrotestNorthWallSceneV1 extends Phaser.Scene {
    constructor() {
      super('microtest-northwall-v1');
      this.dir = 'up';
      this.walkT = 0;
      this.moving = false;
    ...
    create() {
      ...
      if (!this.playerSession) throw new Error('M002.2B.0.1 exige sessão de personagem.');
      ...
      const test = RagbiaMicrotestCollisionV1.selfTest();
      if (!test.ok) throw new Error(`Autoteste M002.2B.0.1 falhou: ${test.errors.join(' | ')}`);
      if (window.RagbiaBoot) window.RagbiaBoot.ok('M002.2B.0.1 — microteste carregado');
    }

    buildTextures() {
      this.textures.addCanvas('microtest-base-v1', RagbiaMicrotestArtV1.drawBase());
    ...
    buildVisual() {
      this.base = this.add.image(0, 0, 'microtest-base-v1').setOrigin(0, 0).setScale(2).setDepth(0);
      this.shadow = this.add.graphics().setDepth(1);
      this.shadow.fillStyle(0x1c1510, 0.42);
      this.shadow.fillRect(674, 294, 220, 18);
      this.shadow.fillRect(936, 292, 128, 14);
      this.debugGraphics = this.add.graphics().setDepth(3000).setVisible(false);
    }

    buildPlayer() {
      const s = RagbiaMicrotestCollisionV1.spawn;
    ...
    buildHud() {
      ...
      hud.fillStyle(0x07100d, 0.74).fillRoundedRect(14, VIEW_H - 54, 1040, 40, 6);

      this.add.text(30, 27, 'M002.2B.0.1 — PROPORÇÃO E CONTINUIDADE', {
      ...
      this.add.text(30, VIEW_H - 45,
        'Mover: WASD/Setas/analógico   C: colisão   R: reset   F: tela cheia   |   Janela/lareira reescaladas',
    ...
    playerFootY() {
      return this.player ? this.player.y + RagbiaMicrotestCollisionV1.PLAYER_FOOT_OFFSET_Y : 0;
    }
    ...
    renderDebug() {
      ...
      for (const s of RagbiaMicrotestCollisionV1.shapes) g.strokeRect(s.x, s.y, s.w, s.h);
      g.lineStyle(2, 0x6fe5ff, 0.98);
      g.strokeCircle(this.player.x, this.playerFootY(), RagbiaMicrotestCollisionV1.PLAYER_RADIUS);
      g.lineStyle(1, 0xca72ff, 0.75);
      g.lineBetween(0, 288, VIEW_W, 288);
    }
    ...
    update(_time, delta) {
      ...
      if (this.moving) {
        x /= len; y /= len;
        const speed = 175;
        const result = RagbiaMicrotestCollisionV1.move(this.player.x, this.player.y, x * speed * dt, y * speed * dt);
  ...
  const config = {
    ...
    scene: MicrotestNorthWallSceneV1
  };
  ...
  function setupStartScreen() {
    ...
    if (window.RagbiaBoot) window.RagbiaBoot.ready('M002.2B.0.1 — tela pronta');
  }
