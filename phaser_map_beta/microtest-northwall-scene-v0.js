(() => {
  ...
  class MicrotestNorthWallScene extends Phaser.Scene {
    constructor() {
      super('microtest-northwall-v0');
      this.dir = 'up';
      this.walkT = 0;
      this.moving = false;
    ...
    create() {
      ...
      if (!this.playerSession) throw new Error('M002.2B.0 exige sessão de personagem.');
      ...
      const test = RagbiaMicrotestCollisionV0.selfTest();
      if (!test.ok) throw new Error(`Autoteste M002.2B.0 falhou: ${test.errors.join(' | ')}`);

      if (window.RagbiaBoot) {
        window.RagbiaBoot.ok('M002.2B.0 — microteste autoral carregado');
      }
    }

    buildTextures() {
      // Cenário desenhado do zero em Canvas lógico 640x360 e ampliado 2x por nearest-neighbor.
      this.textures.addCanvas('microtest-base-v0', RagbiaMicrotestArtV0.drawBase());
      this.textures.addCanvas('microtest-woodpile-v0', RagbiaMicrotestArtV0.drawWoodpile());

      for (const dir of DIRS) {
        this.textures.addCanvas(`apprentice-${dir}-idle`, RagbiaApprenticeArtV0.render(dir, 0, false));
        for (let f = 0; f < 3; f++) {
          this.textures.addCanvas(`apprentice-${dir}-walk-${f}`, RagbiaApprenticeArtV0.render(dir, f, true));
        }
      }
    }

    buildVisual() {
      this.base = this.add.image(0, 0, 'microtest-base-v0')
        .setOrigin(0, 0)
        .setScale(2)
        .setDepth(0);

      // Único prop separado para testar depth real, não recorte de ilustração.
      this.woodpile = this.add.image(1070, 354, 'microtest-woodpile-v0')
        .setOrigin(0.5, 1)
        .setScale(2)
        .setDepth(354);

      // Sombra simples do hearth no piso, desenhada pelo runtime.
      this.sceneShadow = this.add.graphics().setDepth(1);
      this.sceneShadow.fillStyle(0x1d1510, 0.42);
      this.sceneShadow.fillRect(706, 334, 246, 18);

      this.debugGraphics = this.add.graphics().setDepth(3000).setVisible(false);
    }

    buildPlayer() {
      const s = RagbiaMicrotestCollisionV0.spawn;
    ...
    buildHud() {
      ...
      hud.fillStyle(0x07100d, 0.78).fillRoundedRect(14, 14, 520, 90, 8);
      hud.lineStyle(2, 0x6f9475, 0.75).strokeRoundedRect(14, 14, 520, 90, 8);
      hud.fillStyle(0x07100d, 0.74).fillRoundedRect(14, VIEW_H - 54, 970, 40, 6);

      this.add.text(30, 27, 'M002.2B.0 — MICROTESTE AUTORAL', {
        fontFamily: 'Consolas, monospace', fontSize: '22px', color: '#f0ead7', fontStyle: 'bold'
      ...
      this.add.text(30, VIEW_H - 45,
        'Mover: WASD/Setas/analógico   C: colisão   R: reset   F: tela cheia   |   Sem asset ilustrado',
    ...
    playerFootY() {
      return this.player
        ? this.player.y + RagbiaMicrotestCollisionV0.PLAYER_FOOT_OFFSET_Y
        : 0;
    }
    ...
    renderDebug() {
      ...
      for (const s of RagbiaMicrotestCollisionV0.shapes) {
      ...
      g.strokeCircle(
        this.player.x,
        this.playerFootY(),
        RagbiaMicrotestCollisionV0.PLAYER_RADIUS
      );

      g.lineStyle(1, 0xca72ff, 0.75);
      g.lineBetween(0, this.woodpile.depth, VIEW_W, this.woodpile.depth);
    }
    ...
    update(_time, delta) {
      ...
      if (this.moving) {
        x /= len; y /= len;
        const speed = 175;
        const result = RagbiaMicrotestCollisionV0.move(
          this.player.x, this.player.y,
          x * speed * dt, y * speed * dt
        );
      ...
      const tex = this.moving
        ? `apprentice-${this.dir}-walk-${frame}`
        : `apprentice-${this.dir}-idle`;

      if (this.player.texture.key !== tex) this.player.setTexture(tex);

      // Depth pelo footprint. A pilha de lenha é um objeto real separado,
      // então o jogador pode passar visualmente na frente ou atrás dela.
      this.player.setDepth(this.playerFootY());
  ...
  function startSession(rawName) {
    ...
    if (!validation.ok) {
      throw new Error(`Estado inicial inválido: ${validation.errors.join(' | ')}`);
    }
  ...
  function setupStartScreen() {
    ...
    if (!screen || !input || !button || !error) {
      throw new Error('Tela inicial do microteste incompleta.');
    }
    ...
    if (window.RagbiaBoot) {
      window.RagbiaBoot.ready('M002.2B.0 — tela do microteste pronta');
    }
  }
